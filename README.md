...
* `remote_llm_url` / `remote_tts_url`: Set these to your Colab tunneling endpoints for cloud offloading.
  * `remote_llm_url` supports both JSON-chat envelopes and plain-text bridge responses for non-stream calls (candidate/critic stages). **Note:** Summarization runs exclusively on the local model and does not use `remote_llm_url`.
